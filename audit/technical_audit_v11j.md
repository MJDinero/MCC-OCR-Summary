...
+  5. ⚠️ **Hard-coded GCS buckets:** Batch OCR helpers and the PDF splitter always use `demo-gcp-project-*`, blocking reuse across projects and risking data exfiltration to the wrong bucket.【F:src/services/docai_batch_helper.py†L20-L107】【F:src/utils/pdf_splitter.py†L30-L118】
...
+- **Batch processing:** Large documents trigger `batch_process_documents_gcs` and optionally `split_pdf_by_page_limit`, but both utilities hard-code intake/output buckets (`demo-gcp-project-intake/output`).【F:src/services/docai_batch_helper.py†L20-L123】【F:src/utils/pdf_splitter.py†L30-L118】
