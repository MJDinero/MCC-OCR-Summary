...
- **Rationale:** Reaffirmed `.github/AGENTS.md` as the sole authoritative agent spec, brought the smoke test forward to the SummaryContract + validator era, removed the unused `typing.cast` import that broke ruff, replaced the deprecated `actions/upload-artifact` pin so CI can execute gates, and made the hybrid repository serialize contract payloads to deterministic JSON strings for both BigQuery and GCS sinks with a regression test protecting the behavior.
