...
- **Files:** .github/AGENTS.md, .github/workflows/ci.yml, requirements.txt, scripts/smoke_test.py, src/models/summary_contract.py, src/services/summary_repository.py, tests/test_summary_repository.py, docs/audit/HARDENING_LOG.md
- **Rationale:** Reaffirmed `.github/AGENTS.md` as the sole authoritative agent spec, brought the smoke test forward to the SummaryContract + validator era, removed the unused `typing.cast` import that broke ruff, ensured CI provisions a mock ADC file, replaced the deprecated `actions/upload-artifact` pin, added a deterministic service-account fixture plus `pypdf==6.1.3` so gates can execute, and made the hybrid repository serialize contract payloads to deterministic JSON strings for both BigQuery and GCS sinks with a regression test protecting the behavior.
