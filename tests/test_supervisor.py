...
def _make_summary(body: str, diagnoses: list[str], providers: list[str], meds: list[str]) -> dict[str, str]:
    return {
...
def test_supervisor_retry_merges_high_volume_document(supervisor_module, supervisor):
    ...
    pages = [{'page_number': i, 'text': 'Hypertension management plan'} for i in range(1, 251)]
    file_bytes = b'x' * (12 * 1024 * 1024)
    doc_stats = supervisor.collect_doc_stats(text=source_text, pages=pages, file_bytes=file_bytes)
    ...
    outputs = [
        _make_summary(short_body, ['Essential hypertension'], ['Dr. Patel'], ['Lisinopril']),
...
def test_supervisor_pass_small_document_without_retry(supervisor):
    ...
    doc_stats = supervisor.collect_doc_stats(text=text, pages=pages, file_bytes=b'x' * 2048)
    body = (
        "Provider Seen:\nDr. Lane\n\n"
    ...
    summary = _make_summary(body, ['Essential hypertension'], ['Dr. Lane'], ['Losartan 50mg'])
...
def test_strip_section_headers_and_alignment_shortcuts(supervisor_module, supervisor):
    ...
    assert supervisor_module._strip_section_headers('Plain line without colon') == 'Plain line without colon'
...
def test_normalise_mb_and_extract_summary_text(supervisor_module):
    ...
    joined = supervisor_module._extract_summary_text({'Other': 'Alpha', 'Notes': 'Beta'})
    assert 'Alpha' in joined and 'Beta' in joined
    assert supervisor_module._extract_summary_text({'Medical Summary': 'Primary narrative', 'Other': 'Beta'}) == 'Primary narrative'
...
def test_retry_and_merge_returns_initial_when_already_valid(supervisor):
    ...
    pages = [{'page_number': 1, 'text': text}]
    doc_stats = supervisor.collect_doc_stats(text=text, pages=pages, file_bytes=b'x' * 1024)
    body = (
        "Provider Seen:\nDr. Lane\n\n"
    ...
    outputs = [_make_summary(body, ['Essential hypertension'], ['Dr. Lane'], ['Losartan 50mg', 'Amlodipine 5mg'])]
    summariser = SequenceSummariser(outputs)
    initial_summary = summariser.summarise(text)
    initial_validation = supervisor.validate(ocr_text=text, summary=initial_summary, doc_stats=doc_stats)
...
def test_tokenization_filters_stopwords_and_digits(supervisor):
    tokens = list(supervisor._tokenize('The patient 123 presented with hypertension follow up plan.'))
...
def test_paragraph_and_header_counts(supervisor):
    ...
    headers_text = 'Diagnoses: Hypertension\nProvider Seen: Dr. A\nMedications: Lisinopril'
...
def test_validate_handles_zero_target_length(supervisor_module):
    zero_supervisor = supervisor_module.CommonSenseSupervisor(baseline_min_chars=0, multi_pass_min_chars=0)
