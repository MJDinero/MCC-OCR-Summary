...
class DummyChunker:
    async def chunk_async(self, pages):
        yield Chunk(text="hello world", index=0, page_start=1, page_end=1, token_count=3)
...
async def test_ocr_service_records_metrics(monkeypatch):
    ...
    monkeypatch.setattr(service, "_iterate_pages", _fake_pages.__get__(service, OCRService))
    monkeypatch.setattr(service, "_publish_chunk", _noop_publish_chunk.__get__(service, OCRService))
...
async def test_ocr_service_failure_sends_to_dlq(monkeypatch):
    ...
    monkeypatch.setattr(service, "_publish_chunk", raising_publish_chunk.__get__(service, OCRService))
