...
os.environ.setdefault(
    "SIMPLECOV_FOCUS", "src/services/supervisor.py,src/services/drive_client.py"
)
...
def _base_env(monkeypatch):
    ...
    monkeypatch.setenv(
        "DOC_AI_PROCESSOR_ID", os.environ.get("DOC_AI_PROCESSOR_ID", "pid")
    )
    monkeypatch.setenv(
        "DOC_AI_SPLITTER_PROCESSOR_ID",
        os.environ.get("DOC_AI_SPLITTER_PROCESSOR_ID", "splitter"),
    )
    monkeypatch.setenv("OPENAI_API_KEY", os.environ.get("OPENAI_API_KEY", "dummy"))
    monkeypatch.setenv(
        "DRIVE_INPUT_FOLDER_ID", os.environ.get("DRIVE_INPUT_FOLDER_ID", "in-folder")
    )
    monkeypatch.setenv(
        "DRIVE_REPORT_FOLDER_ID", os.environ.get("DRIVE_REPORT_FOLDER_ID", "out-folder")
    )
    monkeypatch.setenv(
        "INTAKE_GCS_BUCKET", os.environ.get("INTAKE_GCS_BUCKET", "intake-bucket")
    )
    monkeypatch.setenv(
        "OUTPUT_GCS_BUCKET", os.environ.get("OUTPUT_GCS_BUCKET", "output-bucket")
    )
    monkeypatch.setenv(
        "SUMMARY_BUCKET", os.environ.get("SUMMARY_BUCKET", "summary-bucket")
    )
    monkeypatch.setenv(
        "SIMPLECOV_FOCUS",
        os.environ.get("SIMPLECOV_FOCUS", "src/services/supervisor.py"),
    )
    yield
...
class _SimpleCoveragePlugin:
    ...
    def pytest_sessionfinish(self, session: pytest.Session, exitstatus: int) -> None:
        ...
        if terminal and any(report == "term-missing" for report in self.reports):
            ...
            for path, total, executed, missing in files_data:
                ...
                terminal.write_line(
                    f"  {rel}: {pct:6.2f}% ({executed}/{total}) Missing: {missing_str}"
                )
            terminal.write_line(
                f"TOTAL: {coverage_pct:6.2f}% ({executed_statements}/{total_statements})"
            )
        ...
        for target in self.targets:
            if target.is_file() and target.suffix == ".py":
                ...
                for candidate in target.rglob("*.py"):
                    if candidate.resolve() in executed_files and self._should_include(
                        candidate
                    ):
                        yield candidate
    ...
    def _should_include(self, candidate: Path) -> bool:
        ...
        return any(
            rel.match(pattern) or rel_str.startswith(pattern.rstrip("*"))
            for pattern in self.focus_patterns
        )

    def _file_stats(
        self, path: Path, counts: dict[tuple[str, int], int]
