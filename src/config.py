...
class AppConfig(BaseSettings):
    ...
    run_pipeline_inline_raw: str | bool | None = Field(
        True, validation_alias="RUN_PIPELINE_INLINE"
    )
    pipeline_workflow_name: str | None = Field(
    ...
    ocr_topic: str = Field(
        "projects/demo/topics/ocr-topic", validation_alias="OCR_TOPIC"
    )
    summary_topic: str = Field(
        "projects/demo/topics/summary-topic", validation_alias="SUMMARY_TOPIC"
    )
    storage_topic: str = Field(
        "projects/demo/topics/storage-topic", validation_alias="STORAGE_TOPIC"
    )
    ocr_subscription: str = Field(
        "projects/demo/subscriptions/ocr-sub", validation_alias="OCR_SUBSCRIPTION"
    )
    summary_subscription: str = Field(
        "projects/demo/subscriptions/summary-sub",
        validation_alias="SUMMARY_SUBSCRIPTION",
    )
    storage_subscription: str = Field(
        "projects/demo/subscriptions/storage-sub",
        validation_alias="STORAGE_SUBSCRIPTION",
    )
    ocr_dlq_topic: str = Field(
        "projects/demo/topics/ocr-dlq", validation_alias="OCR_DLQ_TOPIC"
    )
    summary_dlq_topic: str = Field(
        "projects/demo/topics/summary-dlq", validation_alias="SUMMARY_DLQ_TOPIC"
    )
    storage_dlq_topic: str = Field(
        "projects/demo/topics/storage-dlq", validation_alias="STORAGE_DLQ_TOPIC"
    )
    cmek_key_name: str | None = Field(None, validation_alias="CMEK_KEY_NAME")
    pipeline_state_kms_key: str | None = Field(
        None, validation_alias="PIPELINE_STATE_KMS_KEY"
    )
    enable_diag_endpoints_raw: str | bool | None = Field(
        False, validation_alias="ENABLE_DIAG_ENDPOINTS"
    )
    ...
    summary_bigquery_dataset: str = Field(
        "mcc_summary", validation_alias="SUMMARY_BIGQUERY_DATASET"
    )
    summary_bigquery_table: str = Field(
    ...
    def validate_required(self) -> None:
        ...
        optional_pairs = [
            (
                "drive_impersonation_user",
                self.drive_impersonation_user,
                "DRIVE_IMPERSONATION_USER",
            ),
            ("cmek_key_name", self.cmek_key_name, "CMEK_KEY_NAME"),
            (
                "pipeline_state_kms_key",
                self.pipeline_state_kms_key,
                "PIPELINE_STATE_KMS_KEY",
            ),
        ...
        if raw_credentials.startswith("{"):
            ...
            if not cred_path.is_file():
                raise RuntimeError(
                    f"GOOGLE_APPLICATION_CREDENTIALS path is not a file: {cred_path}"
                )
